...
- Quando um filtro recebe valor fora do **domínio** dele (ex: `resiste_a="fogo"`, sendo que o banco guarda `fire`), levanta `ValorDeFiltroInvalido` informando o parâmetro e o valor recebido.
- **A validação é por domínio, não por filtro isolado.** `DOMINIOS_DE_FILTRO` agrupa os filtros que compartilham valores: os três de dano (`imune_a`/`resiste_a`/`vulneravel_a`) e os dois de condição (`imune_a_condicao`/`impoe`). `imune_a="radiant"` num banco onde ninguém é imune a radiant devolve **vazio**, não erro — `radiant` é um tipo de dano legítimo, e "ninguém é imune a ele" é pergunta coerente sem resposta, exatamente como o intervalo de desafio invertido. Só valor que não existe em domínio nenhum é engano de quem escreveu.
- Corrigido em 2026-07-26, ao implementar a Spec 8: validar por filtro fazia `/vocabulario` **mentir**. O contrato publica seis chaves de domínio, então o endpoint anunciava `radiant` em `tipos_dano` enquanto `imune_a=radiant` respondia 422 — no endpoint que existe justamente para o consumidor não precisar adivinhar. No SRD real eram 9 combinações nessa situação.
- `vocabulario_por_dominio` devolve as seis chaves do contrato, sem contagem. Mora aqui, e não na rota, porque é **o mesmo agrupamento** que a validação usa: se a API tivesse o seu, os dois divergiriam e o endpoint voltaria a anunciar valor que o filtro rejeita.

...
**Ordem:** 7a de 9
**Depende de:** Specs 1-6 (todas concluídas)
**Score:** 4
**Revisão:** pendente
...
- Filtros aceitos, todos opcionais: `tipo`, `tamanho`, `alinhamento`, `desafio_min`, `desafio_max`, `ambiente`, `resiste_a`, `imune_a`, `vulneravel_a`, `imune_a_condicao`, `impoe`, `nome`. Filtro ausente ou vazio não restringe nada.
...
- `ordenar_por` é resolvido por lista branca: para monstros, `nome`, `desafio`, `pontos_vida`, `classe_armadura`, `dano_medio`; para ataques, `bonus_ataque` e `dano_medio`. Valor fora da lista **não** levanta erro — cai na ordenação padrão (`nome` para monstros, `bonus_ataque` decrescente para ataques). Link editado à mão não deve derrubar página.
...
- Quando o resultado tem dano, a média é calculada **em Python** sobre as linhas, via `media_de_dado`. Fica fora do SQL porque o SQLite não interpreta expressão de dado.
- O `dano_medio` **de um monstro** é a média das médias dos ataques dele; monstro sem ataque com dado válido recebe `None`. O `dano_medio` **de um ataque** é a média daquele ataque, e `dano_extra_medio` a do dano secundário.
...
- Quando recebe uma conexão, `vocabulario` devolve, para cada filtro, os valores distintos **com a contagem de monstros de cada um** — `dragon` com 43, `forest` com 151. A tela usa a contagem no rótulo das opções; a API expõe só os valores.
...
- `METRICAS` é a lista branca das colunas calculadas do modo `comparacao` e do `resumo`: contagem de monstros, médias de pontos de vida, classe de armadura, desafio e dano, e **média de bônus de ataque**. Não é escolhida pelo usuário — a tela mostra o conjunto pronto. Existe como lista branca porque é ela que `FiltroDesconhecido` protege.
...
- [ ] Teste confirma que `vocabulario` devolve cada valor com sua contagem.
...
- `bestiario/__init__.py` — acrescenta as re-exportações da API pública nova. Só re-exportação.

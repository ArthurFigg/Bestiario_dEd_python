...
- Quando o usuário clica num cabeçalho de coluna, a lista reordena por aquela coluna. A ordenação vai na URL (`?ordenar=desafio&sentido=desc`), então o estado sobrevive a recarregar e o link é compartilhável.
...
- `tests/web/conftest.py` define **uma** fixture de banco em memória, no schema da Spec 3, com um conjunto pequeno e conhecido de monstros que cobre os casos das três abas: tipos variados, monstro com vários ambientes, monstro com imunidade, monstro com ataque e efeito, monstro sem ataque nenhum.
...
- `web/app.py` — NOVO. Cria a aplicação, configura o Jinja2, serve os estáticos, **inclui o roteador da Spec 8** com o prefixo `/api/v1` e registra as rotas do site, essas fora do esquema da documentação.
- `tests/web/conftest.py` — NOVO. A fixture de banco em memória compartilhada pelas três abas.
...
- `api/` — a Spec 8 é montada como está. Se o site precisar de algo que a API não dá, isso se resolve no núcleo ou numa rota do site, nunca alterando o contrato da API.
...
- Modo "Completa" nesta aba abre só a linha clicada → 325 blocos de estatística empilhados dariam uma página que ninguém lê e que o navegador sofreria para montar.
